...
<script>
    ...
    import {baseURL, loggedAdmin} from '../store/store.js'
    ...
    const handleSubmit = async () => {
        isLoading = !isLoading
        let url = $baseURL+'/login/a'
        try {
            const resp = await axios.post(url, {password, username})
            localStorage.clear()
...
<div
    ...
    <div transition:slide class="mt-5 p-7 bg-blue-803 shadow-lg rounded space-y-5">
        <div>
            <h1 class="text-white text-3xl font-light">Admin</h1>
            <h3 class="text-gray-300 text-sm">Selamat Datang di Halaman Admin Queue Apps</h3>
        </div>
